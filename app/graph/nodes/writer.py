...
async def write_draft_node_streaming(state: AgentState) -> Dict[str, Any]:
    """
    文章写作节点的流式版本
    
    这个版本使用生成器方式调用 LLM，适合配合 astream_events 实现
    真正的 token 级别流式输出
    
    注意：由于 LangGraph 节点返回值需要是完整的状态更新，
    真正的流式输出需要在 API 层通过 astream_events 实现
    ...
    with MetricsContext("write_draft") as tracker:
        try:
            ...
            async for chunk, usage in llm_service.stream_write_draft_generator(
                topic=selected_topic,
                feedback=review_feedback,
                revision_count=revision_count
            ):
